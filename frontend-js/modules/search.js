...
export default class Search {
    ...
    renderResultsHTML(posts) {
      // If you use "Gravatar", you need to replace to src with "post.author.avator".
      if (posts.length) {
        ...
        ${posts.map(post => {
          ...
          <img class="avatar-tiny" src="https://gravatar.com/avatar/b642b4217b34b1e8d3bd915fc65c4452?s=128"> <strong>${post.title}</strong>
          <span class="text-muted small">by ${post.author.username} on ${postDate.getMonth() + 1}/${postDate.getDate()}/${postDate.getFullYear()}</span>
        </a>`
        }).join('')}
      </div>`)
      } else {
        this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">Sorry, we could not find any relsuts for that search.</p>`
      }
      this.hideLoaderIcon()
